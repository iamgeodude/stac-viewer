...
A SvelteKit SPA (Svelte 5 runes) front end for a STAC API. Homepage lists collections (filterable card grid); `/collections/[id]` shows collection metadata, a MapLibre map of items, query filters (date range, limit, map-bounds bbox), a paginated list of per-item **cards** (each with its own expandable assets table), an asset key/role multi-select filter, and a queue-based downloader; `/downloadQueue` manages the persistent download queue, history, and dead-letter. A floating `DownloadWidget` (Start/Pause/Resume + remaining count) shows on every page.
...
- Node 24 / npm 11 on the dev machine. Now a git repo (`master`, single initial commit at the start of this session).
